...
IAnimatedMesh* COBJMeshFileLoader::createMesh(io::IReadFile* file)
{
    ...
    float AverageY = 0;
    float AverageZ = 0;
    core::vector3df vertexBufferTemp[50000];

	while(bufPtrTemp != bufEnd)
	{
		switch(bufPtrTemp[0])
		{
			case 'v':               // v, vn, vt
			...
			case ' ':          // vertex
				{
                    ...
                    core::vector3df vec;
					bufPtrTemp = readVec3(bufPtrTemp, vec, bufEnd);
					
					vertexBufferTemp[vCount] = vec;
					AverageX += vec.X;
					AverageY += vec.Y;
					AverageZ += vec.Z;
		...
		// eat up rest of line
		bufPtrTemp = goNextLine(bufPtrTemp, bufEnd);
	...
	{
		vertexBufferTemp[i].X -= AverageX; 
		vertexBufferTemp[i].Y -= AverageY;
		vertexBufferTemp[i].Z -= AverageZ;
		vertexBuffer.push_back(vertexBufferTemp[i]);
	}
...
const c8* COBJMeshFileLoader::readUV(const c8* bufPtr, core::vector2df& vec, const c8* const bufEnd)
{
	...
	bufPtr = goAndCopyNextWord(wordBuffer, bufPtr, WORD_BUFFER_LENGTH, bufEnd);
	vec.X=core::fast_atof(wordBuffer);  // To DO: LIMIT THIS TO 32 ?!?!
	
	if (vec.X >= 32) vec.X = (f32)31.9;
	else if (vec.X <= -32) vec.X = (f32)-31.9;
	...
	if (vec.Y >= 32) vec.X = (f32)31.9;
	else if (vec.Y <= -32) vec.X = (f32)-31.9;

...
void CSceneNodeAnimatorCameraMaya::animateNode(ISceneNode *node, u32 timeMs)
{
	...
	if ( (isMouseKeyDown(0) && isMouseKeyDown(2)) || isMouseKeyDown(1) )
	{
		...
		else
		{
			...
			nZoom += (ZoomStart.X - MousePos.X) * ZoomSpeed * 100;
	...
	if (isMouseKeyDown(2) && !Zooming)
	{
		...
		{
			translate +=  tvectX * (TranslateStart.X - MousePos.X)*TranslateSpeed*7+  
			              tvectY * (TranslateStart.Y - MousePos.Y)*TranslateSpeed*7;
		}
	}
	else if (Translating)
	{
	...
	if (isMouseKeyDown(0) && !Zooming)
	{
		...
		{
			nRotX += (RotateStart.X - MousePos.X) * RotateSpeed*0.5f;
			nRotY += (RotateStart.Y - MousePos.Y) * RotateSpeed*0.5f;
		}
	}
	else if (Rotating)
	{
		RotX += (RotateStart.X - MousePos.X) * RotateSpeed*0.5f;
		RotY += (RotateStart.Y - MousePos.Y) * RotateSpeed*0.5f;
		nRotX = RotX;
		nRotY = RotY;
		Rotating = false;
	}
